...
const HomePage = () => {
  return (
    <div className="h-screen w-screen bg-space ">
      ...
      <div
        className="w-screen h-screen overflow-hidden absolute top-0 left-0"
        style={{ backgroundImage: "url('Md-stars.png')" }}
      />
      <div
        className="w-screen h-screen overflow-hidden absolute top-0 left-0"
        style={{ backgroundImage: "url('Sm-stars.png')" }}
      />

      <div className="flex justify-center items-center text-8xl font-semibold text-white h-sm pt-20">
      ...
      <div
        className="w-screen absolute bottom-0 left-0 overflow-hidden h-earth bg-cover"
        style={{ backgroundImage: "url('Earth_WH.png')" }}
      />
          ...
          <div className="flex">
            {links.map((currLink) => (
              <Link key={currLink} href={`/${currLink}`}>
                <a className="z-50 p-3 m-2 text-xl border-white border-2 hover:bg-white hover:text-black rounded-md">
                  {currLink}
                </a>
              </Link>
            ))}
